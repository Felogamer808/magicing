"use client";

import { useId } from "react";

interface ZapataDiagramaProps {
  AM: number;
  BM: number;
  anchoPilarAM: number;
  anchoPilarBM: number;
  numeroA: number;
  numeroB: number;
}

const fmtM = (n: number) => `${n.toLocaleString("es-AR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} m`;

function lineasEspaciadas(n: number, desde: number, hasta: number): number[] {
  if (n <= 1) return [(desde + hasta) / 2];
  const paso = (hasta - desde) / (n - 1);
  return Array.from({ length: n }, (_, i) => desde + i * paso);
}

export function ZapataDiagrama({ AM, BM, anchoPilarAM, anchoPilarBM, numeroA, numeroB }: ZapataDiagramaProps) {
  const arrowId = useId();

  if (!(AM > 0) || !(BM > 0)) return null;

  const MAX_W = 230;
  const MAX_H = 210;
  const PAD_LEFT = 46;
  const PAD_TOP = 34;
  const PAD_RIGHT = 20;
  const PAD_BOTTOM = 14;

  const escala = Math.min(MAX_W / AM, MAX_H / BM);
  const w = AM * escala;
  const h = BM * escala;

  const x0 = PAD_LEFT;
  const y0 = PAD_TOP;
  const x1 = x0 + w;
  const y1 = y0 + h;

  const viewW = x1 + PAD_RIGHT;
  const viewH = y1 + PAD_BOTTOM;

  const pilarW = Math.min(anchoPilarAM * escala, w * 0.9);
  const pilarH = Math.min(anchoPilarBM * escala, h * 0.9);
  const pilarX0 = x0 + (w - pilarW) / 2;
  const pilarY0 = y0 + (h - pilarH) / 2;

  const margen = Math.min(10, w * 0.05, h * 0.05);

  // Barras dirección A: corren paralelas a A (horizontal), espaciadas a lo largo de B.
  const ysA = lineasEspaciadas(numeroA, y0 + margen, y1 - margen);
  // Barras dirección B: corren paralelas a B (vertical), espaciadas a lo largo de A.
  const xsB = lineasEspaciadas(numeroB, x0 + margen, x1 - margen);

  return (
    <div className="flex flex-col items-center gap-3">
      <svg
        viewBox={`0 0 ${viewW} ${viewH}`}
        className="h-auto w-full max-w-xs text-primary"
        fill="none"
        aria-hidden="true"
      >
        <defs>
          <marker id={arrowId} markerWidth="6" markerHeight="6" refX="3" refY="3" orient="auto">
            <path d="M0 0 L6 3 L0 6 Z" fill="currentColor" />
          </marker>
        </defs>

        {/* cota A */}
        <g stroke="currentColor" strokeWidth="1" opacity="0.75">
          <path d={`M${x0} ${y0 - 16} L${x0} ${y0 - 4}`} />
          <path d={`M${x1} ${y0 - 16} L${x1} ${y0 - 4}`} />
          <path d={`M${x0} ${y0 - 10} L${x1} ${y0 - 10}`} markerStart={`url(#${arrowId})`} markerEnd={`url(#${arrowId})`} />
        </g>
        <text x={(x0 + x1) / 2} y={y0 - 18} textAnchor="middle" className="fill-current font-mono" fontSize="9">
          A = {fmtM(AM)}
        </text>

        {/* cota B */}
        <g stroke="currentColor" strokeWidth="1" opacity="0.75">
          <path d={`M${x0 - 16} ${y0} L${x0 - 4} ${y0}`} />
          <path d={`M${x0 - 16} ${y1} L${x0 - 4} ${y1}`} />
          <path d={`M${x0 - 10} ${y0} L${x0 - 10} ${y1}`} markerStart={`url(#${arrowId})`} markerEnd={`url(#${arrowId})`} />
        </g>
        <text
          x={PAD_LEFT - 22}
          y={(y0 + y1) / 2}
          textAnchor="middle"
          className="fill-current font-mono"
          fontSize="9"
          transform={`rotate(-90 ${PAD_LEFT - 22} ${(y0 + y1) / 2})`}
        >
          B = {fmtM(BM)}
        </text>

        {/* zapata en planta */}
        <rect x={x0} y={y0} width={w} height={h} stroke="currentColor" strokeWidth="2" fill="var(--color-muted)" fillOpacity="0.4" />

        {/* barras dirección A (horizontales) */}
        {ysA.map((y, i) => (
          <path key={`a-${i}`} d={`M${x0 + margen / 2} ${y} L${x1 - margen / 2} ${y}`} stroke="currentColor" strokeWidth="1" opacity="0.55" />
        ))}

        {/* barras dirección B (verticales) */}
        {xsB.map((x, i) => (
          <path key={`b-${i}`} d={`M${x} ${y0 + margen / 2} L${x} ${y1 - margen / 2}`} stroke="currentColor" strokeWidth="1" opacity="0.85" />
        ))}

        {/* pilar (referencia, línea punteada) */}
        <rect x={pilarX0} y={pilarY0} width={pilarW} height={pilarH} stroke="currentColor" strokeWidth="1.5" strokeDasharray="3 2" fill="none" />
      </svg>

      <dl className="grid w-full max-w-xs grid-cols-2 gap-x-4 gap-y-1 font-mono text-xs text-muted-foreground">
        <dt>Armado A</dt>
        <dd className="text-right text-foreground">{numeroA} barras</dd>
        <dt>Armado B</dt>
        <dd className="text-right text-foreground">{numeroB} barras</dd>
        <dt>Pilar</dt>
        <dd className="text-right text-foreground">
          {fmtM(anchoPilarAM)} × {fmtM(anchoPilarBM)}
        </dd>
      </dl>
    </div>
  );
}
